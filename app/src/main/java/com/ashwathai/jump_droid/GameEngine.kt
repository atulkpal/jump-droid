...
class GameEngine(
    ...
    var runMiniBossesDefeated by mutableIntStateOf(0)
    ...
    var activeGameMode by mutableStateOf(GameMode.STANDARD)
    ...
    init {
        altitudeManager.onZoneChanged = { zone ->
            ...
            // Music Unlock Logic
            val trackName = when (zone) {
                AltitudeZone.EARTH -> "bgm_earth"
                AltitudeZone.CLOUD_LAYER -> "bgm_clouds"
                AltitudeZone.UPPER_ATMOSPHERE -> "bgm_atmosphere"
                AltitudeZone.ORBIT -> "bgm_orbit"
                AltitudeZone.THE_FOUNDRY -> "bgm_foundry"
                AltitudeZone.DEEP_SPACE -> "bgm_space"
                AltitudeZone.CHRONO_RIFT -> "bgm_chrono"
                AltitudeZone.VOID -> "bgm_void"
                AltitudeZone.THE_BEYOND -> "bgm_beyond"
                AltitudeZone.STELLAR_GATE -> "bgm_gate"
                AltitudeZone.ANCIENT_CONSTRUCT -> "bgm_construct"
                AltitudeZone.SINGULARITY -> "bgm_singularity"
            }
            progressionManager.unlockMusicTrack(trackName)
        ...
        missionManager.onMissionCompleted = { mission ->
            if (activeGameMode != GameMode.ZEN) {
    ...
    fun getGameStats() = GameStats(
        ...
        maxAltitudeMeters = (runAltitude / 10f).toInt(),
        totalScore = score,
    ...
    fun checkDiscovery(type: DiscoveryType): Boolean {
        ...
        if (isNew && activeGameMode != GameMode.ZEN) {
    ...
    fun handleRewardCollection(reward: ComboReward) {
        ...
        when (reward) {
            is ComboReward.Artifact -> if (activeGameMode != GameMode.ZEN) notificationManager.post(rewardName, NotificationPriority.TACTICAL)
            is ComboReward.AltitudeBoost -> if (activeGameMode != GameMode.ZEN) notificationManager.post(rewardName, NotificationPriority.TACTICAL)
            else -> {}
        }
        when (reward) {
            is ComboReward.Fuel -> {
    ...
    fun checkUnlock(newScore: Int) {
        if (activeGameMode == GameMode.ZEN) return // No unlocks in Zen mode
    ...
    private fun endRun(cause: String) {
        ...
        if (activeGameMode != GameMode.ZEN && score >= progressionManager.highScore) {
            GlobalScope.launch {
    ...
    fun handleLanding(platform: Platform?, yTop: Float) {
        ...
        if (platform != null) {
            ...
            if (!alreadyLanded) {
                if (activeGameMode != GameMode.ZEN) missionManager.updateProgress(MissionType.PLATFORMING) { it.id.startsWith("plat_land_") }
            ...
            when (platform.type) {
                PlatformType.BOOST -> { 
                    ...
                    if (!alreadyLanded) { 
                        spawnBurst(player.x, yTop, 25, SciFiGold, 400f); screenShake = 10f
                        if (activeGameMode != GameMode.ZEN) missionManager.updateProgress(MissionType.PLATFORMING) { it.id == "plat_boost" } 
                    }
                }
                PlatformType.FUEL -> {
                ...
                PlatformType.MOVING -> {
                    ...
                    if (!alreadyLanded && activeGameMode != GameMode.ZEN) {
                ...
                PlatformType.CONVEYOR -> {
                    ...
                    // Carry speed is handled in resolveCollisions for momentum
                }
                else -> {
                    player.velocityY = com.ashwathai.jump_droid.Constants.LANDING_BOUNCE_VELOCITY
                    player.velocityX *= Constants.HORIZONTAL_DAMPING
            ...
            if (rRight > pLeft && rLeft < pRight && rBottom >= pTop && rTop < pBottom) {
                ...
                when {
                    wasAbove && player.velocityY >= 0 -> {
                        ...
                        if (platform.type == PlatformType.CONVEYOR) player.velocityX = platform.speed
                    }
    ...
    fun update(dt: Float) {
        ...
        if (gameState == GameState.ASCENSION_PROTOCOL && bossesSpawned.contains("BOSS_SINGULARITY")) {
            if (threatManager.activeThreats.none { it.definition.id == "BOSS_SINGULARITY" } && activeGameMode != GameMode.ZEN) {
        ...
        // Missions
        if (activeGameMode != GameMode.ZEN && (runAltitude > 10 || totalPlatformLandings > 0)) {
        ...
        repeat(subSteps) {
            ...
            platforms.forEach { p ->
                if (p.isMoving) {
                    p.x += p.speed * sdt
                    if (p.isWrapAround) {
                        if (p.speed > 0 && p.x > screenWidth) {
                            p.x = -p.width
                        } else if (p.speed < 0 && p.x + p.width < 0) {
                            p.x = screenWidth
                        }
                    } else {
