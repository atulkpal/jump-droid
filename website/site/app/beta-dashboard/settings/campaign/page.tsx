      ...
      <div className="space-y-5">
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-3 font-mono text-sm text-white outline-none focus:border-cyan-400/40"
            min={1}
            max={30}
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setConfig({ ...config, maxInvites: parseInt(e.target.value) || 5 })}
            className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-3 font-mono text-sm text-white outline-none focus:border-cyan-400/40"
            min={1}
            max={20}
          />
        ...
        <div>
          <label className="font-mono text-xs text-slate-400 mb-1.5 block">Delay Between Individual Emails (ms)</label>
          <input
            type="number"
            value={config.delayBetweenEmailsMs}
            onChange={(e) => setConfig({ ...config, delayBetweenEmailsMs: parseInt(e.target.value) || 5000 })}
            className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-3 font-mono text-sm text-white outline-none focus:border-cyan-400/40"
            min={100}
            max={30000}
            step={100}
          ...
          <div className="space-y-3">
            ...
            {senderOption === "specific" && (
              <select
                ...
                className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-3 font-mono text-sm text-white outline-none focus:border-cyan-400/40 mt-2"
              >
