    ...
    <section id="gallery" className="px-6 py-20">
      <div className="mx-auto max-w-lg">
        ...
        <div className="grid grid-cols-2 gap-3">
          {SHOTS.map((shot) => (
            <figure key={shot.src} className="rounded-xl overflow-hidden border border-white/10">
              <img src={shot.src} alt={shot.alt} className="aspect-[9/16] w-full object-cover" loading="lazy" />
              <figcaption className="px-3 py-2 text-xs font-semibold text-slate-400">{shot.caption}</figcaption>
            </figure>
          ))}
        </div>
      </div>
